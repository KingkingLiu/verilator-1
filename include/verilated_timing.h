...
// Dynamic scheduling-related includes
#include <functional>
#include <queue>
...
#else
// Using libstdc++, coroutine library is in std namespace
...
private:
    // TYPES
    using DelayedCoro = std::pair<uint64_t, std::coroutine_handle<>>;
    struct Cmp {
...
public:
    ...
    void push(uint64_t time, std::coroutine_handle<> coro) {
    ...
    auto operator[](uint64_t time) {
        const VerilatedLockGuard guard{m_mutex};
        struct Awaitable {
            VerilatedDelayedQueue& queue;  // The queue to push the coroutine to
            uint64_t time;  // Simulation time at which the coroutine should be resumed
...
public:
    ~VerilatedEventToCoroMap() {
        for (auto& p : m_eventSetsToCoros) { p.second.destroy(); }
    }
