...
void V3Error::init() VL_MT_SAFE {
    ...
    for (int i = 0; i < V3ErrorCode::_ENUM_MAX; i++) {
        singleton().m_describedEachWarn[i] = false;
        singleton().m_pretendError[i] = V3ErrorCode{i}.pretendError();
    }
...
bool V3ErrorGuarded::isError(V3ErrorCode code, bool supp) VL_MT_SAFE {
    if (supp) {
        ...
        return true;
    } else if (code < V3ErrorCode::EC_FIRST_WARN || m_pretendError[code]) {
        return true;
    } else {
        return false;
    }
...
void V3ErrorGuarded::vlAbortOrExit() VL_REQUIRES(m_mutex) VL_MT_SAFE {
