...
class LinkDotFindVisitor final : public VNVisitor {
    ...
    void visit(AstClockingItem* nodep) override {
        ...
        std::string varname;
        AstNodeDType* dtypep;
        if (auto* const assignp = nodep->assignp()) {
            auto* const rhsp = assignp->rhsp()->unlinkFrBack();
        ...
        auto* const newvarp = new AstVar{nodep->fileline(), VVarType::MODULETEMP, varname,
    ...
    void visit(AstParseRef* nodep) override {
        ...
        if (m_ds.m_dotPos == DP_MEMBER && VN_IS(m_ds.m_dotp->lhsp(), LambdaArgRef)
            ...
            if (m_inSens && foundp && m_ds.m_dotPos != DP_SCOPE) {
                if (auto* const clockingp = VN_CAST(foundp->nodep(), Clocking)) {
