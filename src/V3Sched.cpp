...
void schedule(AstNetlist* netlistp) {
    ...
    const auto order = [&](const std::string& name,
        ...
        // Remap sensitivities of the input logic to the triggers
        for (auto* lbs : logic) remapSensitivities(*lbs, trigMap);
    ...
    const auto& nbaKit = order("nba", {&logicRegions.m_nba, &logicReplicas.m_nba});
    ...
    const auto& obsKit = orderIfNonEmpty("obs", logicClasses.m_observed);

    // Step 12: Create the 're' region evaluation function
    const auto& reactKit = orderIfNonEmpty("react", logicClasses.m_reactive);
