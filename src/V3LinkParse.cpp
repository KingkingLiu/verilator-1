...
private:
    ...
    void visit(AstClocking* nodep) override {
        ...
        auto* nextItemp = nodep->itemsp();
        for (auto* itemp = nextItemp; itemp; itemp = nextItemp) {
            nextItemp = VN_AS(itemp->nextp(), ClockingItem);
            if (itemp->exprp()) continue;
            if (itemp->skewp()) {
                if (itemp->direction() == VDirection::INPUT) {
                    ...
                    if (auto* const constp = VN_CAST(itemp->skewp(), Const)) {
    ...
    void visit(AstClockingItem* nodep) override {
        if (nodep->direction() == VDirection::OUTPUT) {
            ...
            } else if (auto* const constp = VN_CAST(nodep->skewp(), Const)) {
                if (constp->num().is1Step()) {
