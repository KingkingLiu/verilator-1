...
public:
    ...
    virtual void visit(AstDelay* nodep) override {
        puts("co_await vlSymsp->__Vm_delayedQueue[VL_TIME_Q() + static_cast<uint64_t>(");
        iterateAndNextNull(nodep->lhsp());
        puts(")];\n");
        iterateAndNextNull(nodep->stmtsp());
    }
    virtual void visit(AstEventControl* nodep) override {
        // Would be better to co_await vlSymsp->__Vm_eventDispatcher[{...}] directly, but there's a
        // bug in GCC 11 that prevents us from using an initializer list in a co_await operand:
        // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=98056. Thus we need to introduce a new
        // variable for the event set.
        puts("{ auto events = {");
        iterateAndNextNull(nodep->sensesp());
        puts("}; co_await vlSymsp->__Vm_eventDispatcher[std::move(events)]; }\n");
        iterateAndNextNull(nodep->stmtsp());
    }
