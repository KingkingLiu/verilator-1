...
private:
    ...
    void visit(AstClockingItem* const nodep) override {
        ...
        auto* const skewp = VN_AS(nodep->skewp(), Const);
        ...
        if (nodep->direction() == VDirection::OUTPUT) {
            auto* const skewedRefp = new AstVarRef{flp, varp, VAccess::READ};
            skewedRefp->user1(true);
            auto* const assignp = new AstAssign{flp, exprp->cloneTree(false), skewedRefp};
            if (skewp->isZero()) {
                ...
                // new triggers.
                auto* const forkp = new AstFork{flp, "", assignp};
            ...
            // Ref to the clockvar
            auto* const refp = new AstVarRef{flp, varp, VAccess::WRITE};
            refp->user1(true);
            if (skewp->num().is1Step()) {
                // Assign the sampled expression to the clockvar (IEEE 1800-2017 14.13)
                auto* const sampledp = new AstSampled{flp, exprp->cloneTree(false)};
                ...
                auto* const assignp = new AstAssign{flp, refp, exprp->cloneTree(false)};
                m_clockingp->addNextHere(new AstAlwaysObserved{
                    flp, new AstSenTree{flp, m_clockingp->sensesp()->cloneTree(false)}, assignp});
            } else {
                // Create a queue where we'll store sampled values with timestamps
                auto* const queueDtp = new AstSampleQueueDType{flp, exprp->dtypep()};
                m_netlistp->typeTablep()->addTypesp(queueDtp);
                auto* const queueVarp = new AstVar{
                ...
                auto* const pushp = new AstCMethodHard{
                ...
                auto* const popp = new AstCMethodHard{
    ...
    void visit(AstDelay* nodep) override {
        ...
        auto* const controlp = new AstEventControl{
            nodep->fileline(),
        ...
        auto* const cntVarp = new AstVar{flp, VVarType::BLOCKTEMP, delayName + "__counter",
                                         nodep->findBasicDType(VBasicDTypeKwd::UINT32)};
        auto* const beginp = new AstBegin{flp, delayName + "__block", cntVarp, false, true};
    ...
    void visit(AstNodeVarRef* nodep) override {
        if (auto* const itemp = VN_CAST(nodep->varp()->user1p(), ClockingItem)) {
    ...
    void visit(AstNodeAssign* nodep) override {
        ...
        if (nodep->timingControlp()) {
            ...
            auto* const assignp = new AstAssign{nodep->fileline(), nodep->lhsp()->unlinkFrBack(),
